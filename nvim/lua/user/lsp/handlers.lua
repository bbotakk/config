-- here is all lsp config: keybinds, display opts, etc...
local M = {}
...
M.setup = function()
	...
	local config = {
		virtual_text = true, -- this will display diagnostics in line (hide if you want it clean)
		signs = {
...
-- -- moved this to keybinds.lua for ease of use
-- local function lsp_keymaps(bufnr)
-- 	local opts = { noremap = true, silent = true }
-- 	local map = vim.api.nvim_buf_set_keymap
-- 	map(bufnr, "n", "gD", "<cmd>lua vim.lsp.buf.declaration()<CR>", opts)
-- 	map(bufnr, "n", "gd", "<cmd>lua vim.lsp.buf.definition()<CR>", opts)
-- 	map(bufnr, "n", "H", "<cmd>lua vim.lsp.buf.hover()<CR>", opts)
-- 	map(bufnr, "n", "gI", "<cmd>lua vim.lsp.buf.implementation()<CR>", opts)
-- 	map(bufnr, "n", "gr", "<cmd>lua vim.lsp.buf.references()<CR>", opts)
-- 	map(bufnr, "n", "gl", "<cmd>lua vim.diagnostic.open_float()<CR>", opts)
-- 	map(bufnr, "n", "gn", "<cmd>lua vim.diagnostic.goto_next({buffer=0})<cr>", opts)
-- 	map(bufnr, "n", "gN", "<cmd>lua vim.diagnostic.goto_prev({buffer=0})<cr>", opts)

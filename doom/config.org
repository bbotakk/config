...
- I am generally against utf-symbols as they cannot be typed out with the keyboard -> thus create unneccessary mental overhead and abstraction, to be noted there are exceptions and it is useful to hide the boilerplate code with syntactic sugar in some cases, such as the org formatting with <#+> and the org bullets.
#+begin_src emacs-lisp
(after! org
